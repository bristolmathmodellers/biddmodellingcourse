...
1. A new point of care test for TB has been developed. This test can detect resistance to all first line TB treatments with 100% sensitivitiy (true positive rate) and 50% specificity (true negative rate). What impact will this have on TB incidence?
1. There has been a prolonged shortage of the BCG vaccine, now that there is plentiful supply should a catch up program be implemented for those that missed vaccination? A prolonged shortage of BCG vaccine.
...
The susceptible, exposed (latently infected), infectious and recovered (SEIR) model of TB is the simplest model that can be constructed which still reflects the known dynamics of TB[@Brooks-Pollock2010]. As TB has a long latent period, with cases potentially remaining infected but not actively symptomatic for many years, it is generally neccessary to include demographic processes in any model of TB. 
